 ...
 * 
 */
public class Cal_kdist_simplePruning {
		...
		protected void setup(Context context) throws IOException,
			...
			for (j = 0; j < numOfPivots; j++) {
				...
				for (k = 0; k < numOfPivots; k++) {
					...
					distMatrix[j][k] = metric.dist(pivots.get(j),
						pivots.get(k));
				}
			}
	...
	// output format: partition id       pid,dist(to own pid),nid,lan,long
		public void map(MetricKey key, MetricValue value, Context context)
			...
			for (SortByDist obj : lbs) {
				...
				//	System.out.println("interKey = "+ interKey +"----interValue = "+interValue);
	...
	public void run(String[] args) throws Exception {
		...
		job.setJarByClass(Cal_kdist_simplePruning.class);
	...
	public static void main(String[] args) throws Exception {
		Cal_kdist_simplePruning rs = new Cal_kdist_simplePruning();
		rs.run(args);
	}
}
