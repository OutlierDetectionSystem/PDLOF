...
//			System.out.println("Domain size: " + domains[0][0] + "," + domains[0][1]);
...
//		}
	...
	public static class KCenterSeperateReducer extends Reducer<IntWritable, Text, NullWritable, Text> {
		...
		private static int numOfOutliers = 20; 
		/**
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			numOfOutliers = conf.getInt(SQConfig.strNumOfOutliers,200);
		}
...
//			System.out.println("Number of point in this partition: " + num_data);
...
//			}
		...
		//	pivots.put(allPoints.get(0).getPoint(),num_data);
			 ...
			 */
			while(pivots.size() < numOfPivots + numOfOutliers){
				float maxDist = 0.0f;
				...
				for(int i = 0; i < allPoints.size() ;i++){
					...
					if(distToNewPivot < allPoints.get(i).getDistToPivot()){
						if(allPoints.get(i).getPivotPoint()!=null){
							// update the old pivot (reduce the number of points assigned to that pivot)
...
//				System.out.println("# of pivots =  "+ pivots.size() + ", Max kdistance = " + maxDist +  ", numInNewPivot = " + numOfPointsInNewPivot);
			...
			while(iter.hasNext()) {
			    ...
			    context.write(NullWritable.get(), new Text(keyHt.toString()+","+valueHt));
			}
//			System.out.println("All Points in this partition: " + numOfPointsInside);
		}
	}
